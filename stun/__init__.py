...
def get_nat_type(s, source_ip, stun_host=None, stun_port=3478):
    _initialize()
    port = stun_port
    log.debug("Do Test1")
    resp = False
    if stun_host:
        ret = stun_test(s, stun_host, port)
        resp = ret['Resp']
    else:
            ...
            if not ret['Resp']:
                ...
                if exIP == ret['ExternalIP'] and exPort == ret['ExternalPort']:
                    changePortRequest = ''.join([ChangeRequest, '0004',
                                                 "00000002"])
                    log.debug("Do Test3")
                    ret = stun_test(s, changedIP, port,
                                    changePortRequest)
                    log.debug("Result: %s", ret)
                    if ret['Resp']:
                        typ = RestricNAT
...
def get_ip_info(source_ip="0.0.0.0", source_port=54320, stun_host=None,
    ...
    nat_type, nat = get_nat_type(s, source_ip,
